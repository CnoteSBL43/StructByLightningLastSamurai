...
void	 Swordsman::Render(void)
{
	...
	if (m_facingRight)
		AnimationSystem::GetInstance()->Render(m_Timestamp, p.x, p.y, { -1, 1 });
	else
		AnimationSystem::GetInstance()->Render(m_Timestamp, p.x, p.y, { 1, 1 });
}
...
{
	return AnimationSystem::GetInstance()->GetRect(m_Timestamp, m_ptPosition.x, m_ptPosition.y, { 1, 1 });//SGD::Rectangle(SGD::Point{ m_ptPosition.x - Game::GetInstance()->GetCameraPosition().x - 54.0f, m_ptPosition.y - Game::GetInstance()->GetCameraPosition().y + 6.0f }, SGD::Size{ 45, 99 });
}
